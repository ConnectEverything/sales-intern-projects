...
export async function handler(
  ...
  // inboxes are normally randomly generated by the server, so we create a private inbox
  ...
  const partialUserOpts: Partial<User> = {
    pub: {
      allow: [
        ...
        "$JS.API.CONSUMER.CREATE.rooms", // consume rooms stream
        "$JS.API.STREAM.INFO.KV_bucketOfRooms", // Bind to KV bucket/ stream info for KV_bucketOfRooms
        "$JS.API.CONSUMER.CREATE.KV_bucketOfRooms", // KV watch, creates consumer for KV_bucketOfRooms
        "$JS.API.DIRECT.GET.KV_bucketOfRooms.$KV.bucketOfRooms.*", // Direct Get on KV_bucketOfRooms
        `updateRoom.*.${userData.userName}` // KV service
      ],
      deny: []
    },
    sub: {
  ...
  const expirationDate = Math.floor(
    new Date(dateRn.getTime() + 60 * 60 * 24 * 5 * 1000).getTime() / 1000
  );
