import type {JetStreamClient, KV, NatsConnection} from "https://deno.land/x/nats@v1.13.0/src/mod.ts";
import {DiscardPolicy, RetentionPolicy} from "https://deno.land/x/nats@v1.13.0/src/mod.ts";
...
export class NatsCon {
  ...
  async getKVClient() {
    if (!this.roomBucket) {
      ...
      this.roomBucket = await js.views.kv("bucketOfRooms", { max_bytes: 100000000, maxValueSize: 131072 });
    }
...
export function getServerNatsConnection() {
  return serverNC ??= (async() => {
    ...
    return new NatsCon(await connect({
      servers: 'wss://connect.ngs.global',
      authenticator: jwtAuthenticator(jwt, new TextEncoder().encode(seed)),
      noEcho: true
    }), "server")
